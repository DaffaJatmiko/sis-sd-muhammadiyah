...
<html lang="en">
                  ...
                  $(document).ready(function () {
                      ...
                      $(".mobile-menu-dropdown a").click(function (e) {
                          ...
                          if (targetPage !== "javascript:void(0)") {
                              // Menggunakan window.location.replace untuk menggantikan window.location.href
    ...
    <h1 class="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">Berita</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2">
            ...
            @endphp
            @if($latestNews)
                <img src="../image/{{$latestNews->image}}" alt="{{$latestNews->title}}" class="w-full h-80 md:h-96 object-cover rounded-lg">

                <p class="text-xl mt-4 md:mt-6 pr-5">{{$latestNews->title}}</p>
                <button onclick="location.href='{{ route('detail.publikasi', ['id' => $latestNews->id]) }}'" class="mt-7 px-8 py-4 rounded-md bg-blue-500 text-white transition duration-300 ease-in-out transform hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-300">
                    Selengkapnya
                </button>
            @endif
        </div>
        ...
        <!-- Berita box scroll -->
        <div class="md:mx-auto grid gap-5 max-sm:mt-5 max-h-[calc(100vh-5rem)] overflow-y-auto">
            @foreach($news as $data)
                @if($data->id !== $latestNews->id)
                    <a href="{{ route('detail.publikasi', ['id' => $data->id]) }}" class="grid grid-cols-2 border-b-2 border-gray-300 py-3">
                        <img src="../image/{{$data->image}}" alt="{{$data->title}}" class="w-full md:w-36 h-28 object-cover rounded-lg">
                        <div class="mx-2">
                            <h2 class="font-semibold">{{$data->title}}</h2>
                            <p class="text-sm opacity-70 my-1 font-semibold">{{$data->created_at}}</p>
                        </div>
                    </a>
                @endif
            @endforeach
